...
import type { SessionRecord } from "../core/models.js";

type Blessed = {
  screen: (options: Record<string, unknown>) => BlessedNode;
...
async function runBlessedPicker(blessed: Blessed, sessions: SessionRecord[], initialQuery: string): Promise<void> {
  ...
  function selectedSession(): SessionRecord | undefined {
    return filtered[list.selected ?? 0];
  }

  function setHeader(message = ""): void {
  ...
  function updateList(keepIndex = 0): void {
  ...
  screen.on("keypress", (ch, key) => {
    ...
    if (isKeyRecord(key) && (key.ctrl || key.meta)) return;
    if (isKeyRecord(key) && key.name && ["up", "down", "left", "right", "return", "enter", "escape", "tab", "backspace"].includes(key.name)) return;
    if (ch >= " ") {
      query += ch;
      updateList(0);
    }
...
function loadBlessed(): Blessed | undefined {
  try {
    return require("blessed") as Blessed;
  } catch {
    return undefined;
  }
}

function printSessionList(sessions: SessionRecord[]): void {
  for (const [i, session] of sessions.entries()) {
    console.log(`${String(i + 1).padStart(2, " ")} ${formatDate(session.updatedAt)} ${session.sessionId} ${oneLine(session.title ?? "")}`);
  }
}

function printRawPicker(sessions: SessionRecord[], cursor: number, query: string): void {
...
function formatBlessedListLine(session: SessionRecord): string {
...
function buildBlessedPreview(session: SessionRecord): string {
  return [
...
function formatRawListLine(session: SessionRecord, selected: boolean, width: number): string {
...
function buildPreviewLines(session: SessionRecord, width: number): string[] {
  return [
    "Selected session",
    "-".repeat(width),
...
function isKeyRecord(value: unknown): value is { name?: string; ctrl?: boolean; meta?: boolean } {
...
function cleanForDisplay(value: string): string {
